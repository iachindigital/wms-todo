...
export default function ReturnInventoryPage() {
  ...
  const fetchData = useCallback(async () => {
    setLoading(true)
    try {
      const params = new URLSearchParams({ type: '3', page: String(page), pageSize: '50' })
  ...
  const handleSync = async () => {
    ...
    try {
      const r = await fetch('/api/inventory/sync', {
        ...
        body: JSON.stringify({ inventoryType: 3 }),
      })
      const d = await r.json()
      if (d.success) { setSyncMsg({ text: `✅ 同步成功，共 ${d.synced} 条退货库存`, ok: true }); fetchData() }
    ...
    <div style={{ flex: 1, display: 'flex', flexDirection: 'column', overflow: 'hidden', background: '#f8fafc' }}>
      <div style={{ padding: '14px 24px', background: '#fff', borderBottom: '1px solid #e2e8f0', flexShrink: 0 }}>
        <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between' }}>
          <div>
            <h1 style={{ fontSize: '16px', fontWeight: 700, color: '#0f172a' }}>↩️ 退货库存</h1>
            <p style={{ fontSize: '11px', color: '#94a3b8', marginTop: '2px' }}>FBA退货换标出库 · FNSKU维度{snapDate ? ` · 快照：${snapDate}` : ''}</p>
          </div>
          <div style={{ display: 'flex', gap: '8px', alignItems: 'center' }}>
            <input value={search} onChange={e => { setSearch(e.target.value); setPage(1) }}
              placeholder="搜索SKU/FNSKU..." onKeyDown={e => e.key === 'Enter' && fetchData()}
              style={{ padding: '7px 12px', border: '1px solid #e2e8f0', borderRadius: '6px', fontSize: '12px', outline: 'none' }} />
            <button onClick={handleSync} disabled={syncing}
              style={{ padding: '7px 14px', borderRadius: '7px', background: syncing ? '#e2e8f0' : '#d97706', border: 'none', color: syncing ? '#94a3b8' : '#fff', fontSize: '12px', fontWeight: 600, cursor: syncing ? 'not-allowed' : 'pointer' }}>
              {syncing ? '同步中...' : '↻ 同步退货库存'}
            </button>
          </div>
        </div>
      ...
      <div style={{ flex: 1, overflowY: 'auto', padding: '16px 24px' }}>
        ...
        : data.length === 0 ? (
          <div style={{ textAlign: 'center', padding: '60px', color: '#94a3b8', background: '#fff', borderRadius: '10px', border: '1px solid #e2e8f0' }}>
            ...
            <div>暂无退货库存数据，请点击「同步退货库存」</div>
          </div>
        ) : (
          <div style={{ background: '#fff', borderRadius: '10px', border: '1px solid #e2e8f0', overflow: 'hidden' }}>
            <table style={{ width: '100%', borderCollapse: 'collapse', fontSize: '12px' }}>
              <thead><tr>{['SKU', '产品名称', 'FNSKU', '库位', '仓库', '可用量', '总量'].map(h => <th key={h} style={th}>{h}</th>)}</tr></thead>
              <tbody>
                {data.map((item, i) => (
                  <tr key={item.id} style={{ background: i % 2 === 0 ? '#fff' : '#fafbfc' }}>
                    <td style={td}><span style={{ fontFamily: 'monospace', color: '#d97706', fontWeight: 600 }}>{item.sku}</span></td>
                    <td style={{ ...td, maxWidth: '180px', overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap' }}>{item.sku_name || '-'}</td>
                    <td style={td}><span style={{ fontFamily: 'monospace', fontSize: '11px', background: '#fffbeb', color: '#d97706', padding: '1px 5px', borderRadius: '3px' }}>{item.fnsku || '-'}</span></td>
